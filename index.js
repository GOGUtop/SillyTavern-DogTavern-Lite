 ...
 * 功能：悬浮工具菜单 / 选中生成卡片 / 6 种风格海报 / AI 回复提示音 / 截断空回检测
 * 适配：iOS Safari / Android / PC 浏览器 全通用
 */
...
(function () {
    ...
    // ============ 设置存储 ============
    const defaultSettings = { soundEnabled: true };
    ...
    function playSound() {
        ...
        } catch (e) { console.warn('[DogTavern] 提示音失败', e); }
    }
    ...
    async function drawPosterCard(rawText, charName, avatarUrl, styleIdx) {
        ...
        for (let i = 0; i < 35; i++) {
            ...
            const x = rand() * W, y = rand() * totalH, r = rand() * 2.5 + 0.8;
        ...
        const ax = padding + avatarSize / 2;
        const ay = padding + avatarSize / 2;
        ...
        lines.forEach((line, i) => {
            ctx.fillText(line, padding, textTop + i * lineHeight + 40);
        });
        ...
        lg2.addColorStop(0.5, st.accent);
    ...
    function injectFloatingMenu() {
        ...
        function buildItems() {
            ...
            items.appendChild(makeItem(
                ...
                () => {
                    ...
                    saveSettings();
                    showToast(settings.soundEnabled ? '🔊 AI 提示音已开启' : '🔇 AI 提示音已关闭');
                    if (settings.soundEnabled) playSound();
                }
            ));
            items.appendChild(makeItem('🔖', '选中生成卡片', 'linear-gradient(135deg,#f093fb,#f5576c)', () => {
                showToast('💡 请先选中 AI 消息文字\n然后点击弹出的"生成卡片"按钮', 3500);
            }));
            items.appendChild(makeItem('🎵', '测试提示音', 'linear-gradient(135deg,#fa709a,#fee140)', () => {
                if (!settings.soundEnabled) {
                    showToast('🔇 声音已关闭，请先开启');
                    return;
                }
                playSound();
                showToast('🎵 叮咚~ 这就是 AI 回复提示音');
            }));
            items.appendChild(makeItem('ℹ️', '关于插件', 'linear-gradient(135deg,#11998e,#38ef7d)', showAboutDialog));
        ...
        trigger.addEventListener('touchend', () => {
            if (!dragMoved) { isOpen ? collapse() : expand(); }
        });
        trigger.addEventListener('click', (e) => {
            // 兼容 PC 鼠标点击
            if (!('ontouchstart' in window)) { isOpen ? collapse() : expand(); }
        });

        document.addEventListener('touchstart', (e) => {
            if (isOpen && !folder.contains(e.target)) collapse();
        }, { passive: true });
        ...
        // 5秒自动收回
        setInterval(() => { if (isOpen) collapse(); }, 5000);
    ...
    function showAboutDialog() {
        ...
        wrapper.innerHTML = `
            <div class="dog-modal-panel">
                ...
                <div style="font-size:12px;color:rgba(255,255,255,0.55);text-align:center;margin-bottom:18px;">v1.0.0 · 跨平台增强插件</div>
                <div class="dog-about-card"><b>🐾 悬浮工具菜单</b><br/>点击右侧肉掌展开功能</div>
                <div class="dog-about-card"><b>🔖 选中即生成卡片</b><br/>选中 AI 消息文字，点击浮现的按钮</div>
                <div class="dog-about-card"><b>🎨 6 种精美风格</b><br/>暮光紫/樱花粉/暖茶棕/月光蓝/晚霞橙/墨韵青</div>
                <div class="dog-about-card"><b>🔊 AI 回复提示音</b><br/>回复完成/被截断/空回复三种提示</div>
    ...
    function attachGenerationHooks() {
        let manualStop = false, finishReason = 'unknown', hasError = false;

        // fetch 拦截：捕获 finish_reason
                        ...
                        try {
                            ...
                            while (!done) {
                                ...
                                if (x.value) {
                                    ...
                                    if (ck.includes('"finish_reason":"length"') || ck.includes('"finish_reason": "length"')) finishReason = 'length';
                                    else if (ck.includes('"finish_reason":"stop"') || ck.includes('"finish_reason": "stop"') || ck.includes('"finish_reason":"eos_token"')) finishReason = 'stop';
                                }
                            }
            ...
            new MutationObserver((ms) => {
                ms.forEach(m => m.addedNodes.forEach(n => {
                    if (n.nodeType === 1) {
                        ...
                        if (et && !et._dogHandled) { et._dogHandled = true; hasError = true; window._dogHasError = true; }
                    }
                }));
        ...
        const tryHook = () => {
            ...
            es.on('generation_started', () => {
                manualStop = false; finishReason = 'unknown'; hasError = false;
            ...
            es.on('generation_ended', () => {
                Promise.resolve().then(() => {
                    ...
                    if (isEmpty) {
                        showToast('😾 可恶的 AI！竟然空回本汪！', 3500);
                        playSound(); return;
                    }
                    if (ms || r === 'length') {
                        showToast('😭 呜呜呜！为什么截断我汪', 3500);
                        playSound(); return;
                    }
                    if (r === 'stop') {
    ...
    // ============ 入口 ============
    function init() {
        console.log(`[${PLUGIN_NAME}] 🐶 小狗酒馆 Lite 正在启动...`);
        injectFloatingMenu();
        injectSelectionCard();
